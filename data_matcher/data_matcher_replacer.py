...
backup_board_id = 3260543406
sites_board_id = 3000198404
...
backup_group_sites = {
    backup_group_vals['name']: backup_group_vals['column_values'][0]['text'] for backup_group_vals in backup_group_data}

backup_group = list(enumerate(backup_group_sites.keys()))
backup_sites = list(enumerate(backup_group_sites.values()))

all_site_ids = set((site_val['name'].casefold(), site_val['id']) for site_val in sites_data['items'])
groups_ids = {name: set(int(site_val_id) for site_val_name, site_val_id in all_site_ids
                       if site_val_name.casefold() in set(site_name.casefold() for site_name in sites.split(', ')))
             for name, sites in backup_group_sites.items()}

col_id = "connect_boards44"
connect_new_id=[[query_helper(
    ...
    },0) for group_name in groups_ids if group_id['name'].casefold() in group_name.casefold()]for group_id in group_data]

write_data(backup_group_ingest=backup_group_ingest,
